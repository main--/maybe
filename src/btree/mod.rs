...
impl MappedBTree {
    pub fn open<P: AsRef<Path>>(path: P) -> io::Result<MappedBTree> {
        let mapping = MappedHeap::open(path)?;

        // ensure that the root page is allocated
        let newpage = mapping.alloc();
        if newpage != ROOT_PAGE {
            mapping.free(newpage);
        }

        Ok(MappedBTree { mapping })
    }

    pub fn open_file(file: File) -> io::Result<MappedBTree> {
        let mapping = MappedHeap::open_file(file)?;
        if mapping.page(ROOT_PAGE).is_none() {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "Invalid btree file!"));
        }
    ...
    fn page(&self, id: PageId) -> Option<&BTreePage> {
        unsafe { self.mapping.page_ref(id) }
    }
...
mod tests {
    ...
    #[test]
    fn concurrency() {
        let _ = fs::remove_file("/dev/shm/btree_conc.bin");
        let tree = MappedBTree::open("/dev/shm/btree_conc.bin").unwrap();
        ...
        while let None = tree.get(64) {
        }
        while let Some(i) = tree.get(64) {
            assert_eq!(i, 64);
        }
    ...
    fn it_works(size: u64) {
        let _ = fs::remove_file("/dev/shm/btree.bin");
        let tree = MappedBTree::open("/dev/shm/btree.bin").unwrap();
        let tree2 = MappedBTree::open("/dev/shm/btree.bin").unwrap();
