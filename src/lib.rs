...
impl MappedBTree {
    pub fn open<P: AsRef<Path>>(path: P) -> MappedBTree {
        let mapping = MappedHeap::open(path).unwrap();
    ...
    pub fn open_file(file: File) -> MappedBTree {
...
mod tests {
    ...
    fn it_works(size: u64) {
        let _ = std::fs::remove_file("/dev/shm/btree.bin");
        let tree = MappedBTree::open("/dev/shm/btree.bin");
